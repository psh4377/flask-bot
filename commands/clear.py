...
# setup 함수 정의
def setup(bot):
    if "clear" not in bot.all_commands:
        bot.add_command(
            commands.Command(clear, name="clear", help="지정된 수의 메시지를 삭제합니다."))
        logging.info("Clear command registered successfully.")
    else:
        logging.warning("Clear command is already registered, skipping.")
